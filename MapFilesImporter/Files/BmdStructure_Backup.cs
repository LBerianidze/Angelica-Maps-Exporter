...
namespace MapFilesImporter
{
    ...
    public class BmdStructure
    {
        ...
        public BmdStructure(BinaryReader br, bool change = false)
        {
            ...
            if (br.BaseStream.Length - br.BaseStream.Position > 10)
            {
                var pos = br.BaseStream.Position;
                try
                {
                    var chars = br.ReadChars(2);
    ...
    public class Mesh_Struct
    {
        ...
        public Mesh_Struct(BinaryReader br, int version, short u_3, List<string> AllTextures)
        {
            ...
            if (version2 > 1)
            {
                ...
                for (var i = 0; i < 4; i++)
                {
                    Textures[i] = Encoding.GetEncoding(936).GetString(br.ReadBytes(64)).TrimEnd('\0');
                }
            ...
            else
            {
                for (var i = 0; i < vertexcount; i++)
                {
                    Unknown_structS.Add(new Unknown_struct());
                }
                MeshLocation = new Location();
            }
            if (version == -2147483647)
            {
                //if (version2 != 4)
            ...
            if (u_6 == 1)
            {
                bytes = br.ReadBytes(vertexcount * 8);
            }
            version2 = 6;
        }
